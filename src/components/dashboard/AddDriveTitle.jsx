...
export default function AddDriveType() {
	...
	const handleSubmit = async (e) => {
		...
		try {
			const response = await fetch(`${config.apiEndpoint}/drive-type/`, {
				method: "POST",
				headers: {
	...
	useEffect(() => {
		fetch(`${config.apiEndpoint}/drive-types/`)
			.then((res) => res.json())
			.then((result) => {
				setDriveType(result.data);
			})
						...
						<main id="main" className="main-content">
							<div className="tfcl-dashboard flex">
								<div className="tfcl-add-listing car-details">
									...
									>
										<h1 className="fs-3">All Drive Types</h1>
										{driveType.map((fuelType) => (
											<ol key={fuelType.id}>
