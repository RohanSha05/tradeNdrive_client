...
export default function AddOwnerType() {
	...
	const handleSubmit = async (e) => {
		...
		try {
			const response = await fetch(`${config.apiEndpoint}/owner-type/`, {
				method: "POST",
				headers: {
	...
	useEffect(() => {
		fetch(`${config.apiEndpoint}/owner-types/`)
			.then((res) => res.json())
			.then((result) => {
				setOwnerTypes(result.data);
			})
						...
						<main id="main" className="main-content">
							<div className="tfcl-dashboard flex">
								<div className="tfcl-add-listing car-details">
									...
									>
										<h1 className="fs-3">All Owner Types</h1>
										{ownerTypes.map((fuelType) => (
											<ol key={fuelType.id}>
